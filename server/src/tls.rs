...
pub fn make_config(args: &common::settings::Tls) -> Result<TlsConfig> {
    ...
    for root in ca_certs {
        ...
        if let Some(existing) = ca_thumbprints.get(&issuer) {
            if existing != &thumbprint {
                bail!("Duplicate CA subject with different thumbprints: {}", &issuer);
            }
            // already present, skip, but warn
            warn!("Duplicate CA certificate found for subject: {}, thumbprint: {}", &issuer, &thumbprint);
            continue;
        }
        ca_thumbprints.insert(issuer, thumbprint);
